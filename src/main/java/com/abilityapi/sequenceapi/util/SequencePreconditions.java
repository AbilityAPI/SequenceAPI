...
public class SequencePreconditions {
    ...
    public static <T> void checkOriginType(final SequenceContext sequenceContext, final SequenceContextKey key,
    ...
    public static <T> void checkOriginType(final SequenceContext sequenceContext, final SequenceContextKey key,
                                           final Class<? extends T> type, String message) throws NoSuchElementException {
        if (sequenceContext.get(key) == null) throw new NoSuchElementException("Element for key '" + key.getId() + "' was null.");
